 ...
 where
  ...
  verifyRule ruleLine (Derivation wf wr) = Derivation wf
    ...
    $ do
      ...
      pass
   where
    (formulaText, formula, ruleText, ra@(RuleApplication ruleName refs)) =
      case (wf, wr) of
        (ParsedInvalid formulaText _ f, ParsedInvalid ruleText _ ra) ->
          (formulaText, f, ruleText, ra)
        (ParsedValid formulaText f, ParsedInvalid ruleText _ ra) ->
          (formulaText, f, ruleText, ra)
        (ParsedInvalid formulaText _ f, ParsedValid ruleText ra) ->
          (formulaText, f, ruleText, ra)
        (ParsedValid formulaText f, ParsedValid ruleText ra) ->
          (formulaText, f, ruleText, ra)
    ---------------------------------------------------
    -- 1. Check that the rule exists.
    ...
    unifyReferences
      n
      (RuleSpec (fSpec : fSpecs) pSpecs cSpec)
      (LineReference refLine : refs) = do
    ...
    unifyReferences
      n
      (RuleSpec [] (pSpec : pSpecs) cSpec)
      (ProofReference start end : refs) = do
    ...
    collectTermsTerm ((line, Fun _ ts, TFun _ ss) : rest) =
      collectTermsTerm (zipWith (\t s -> (line, t, s)) ts ss <> rest)
    collectTermsTerm ((line, t, TPlaceholder n) : rest) =
      insertWith
        (<>)
        n
        [(line, t)]
    ...
    collectTermsFormula ((line, Right (Pred _ ps, FPred _ qs)) : rest) =
      M.unionWith
        (<>)
        (collectTermsTerm (zipWith (\p q -> (line, p, q)) ps qs))
        (collectTermsFormula rest)
    collectTermsFormula ((line, Right (Opr _ fs, FOpr _ fs')) : rest) =
      collectTermsFormula (zipWith (\f f' -> (line, Right (f, f'))) fs fs' <> rest)
    collectTermsFormula ((line, Right (Quantifier _ v f, FQuantifier _ v' f')) : rest) =
      insertWith
        (<>)
        v'
        [(line, Var v)]
        (collectTermsFormula ((line, Right (f, f')) : rest))
    collectTermsFormula ((line, Left (FreshVar m, FFreshVar n)) : rest) =
      insertWith (<>) n [(line, Var m)] $ collectTermsFormula rest
    collectTermsFormula ((line, Left (RawAssumption f, AssumptionSpec fSpec)) : rest) =
      collectTermsFormula ((line, Right (f, fSpec)) : rest)
    ...
    verifyTerms m = fromList <$> mapM makeUnique (toPairs m)
     where
      ...
      makeUnique (v, t :| ts) = do
        foldlM
          ( \(_, lastTerm) (currTermLine, currTerm) ->
              if lastTerm == currTerm
                then Right (currTermLine, currTerm)
                else
                  Left $
    ...
    verifyFormulae termMap formsAndSpecs = do
      formMap <-
        reduceFormulae $ M.map (Left <$>) $ collectSimpleFormulae (snd <$> formsAndSpecs)
     ...
     where
      ...
      reduceFormulae = M.traverseWithKey reduceHelper
       where
        ...
        reduceHelper n (Right (fOrig, f :| []) :| rest) = go n f rest
        reduceHelper n (Right (fOrig, f :| (f' : fs)) :| rest) = case go n f rest of
          Left err -> reduceHelper n (Right (fOrig, f' :| fs) :| rest)
          Right f -> Right f
        go ::
          Name ->
          RawFormula ->
            ...
            then
              ...
              Left $
                "Error:\ntrying to verify that all assignments\nof placeholder "
                  ...
                  <> "↦"
                  <> prettyPrint f'
        go n f (Right fs' : rest) = mapFs fs' (snd fs')
         where
          mapFs ::
            ((RawFormula, Subst Term), NonEmpty RawFormula) ->
            NonEmpty RawFormula ->
            Either Text RawFormula
          mapFs ((fOrig, sub), f' :| []) allFs =
            if f /= f'
              then
                Left $
          ...
          mapFs (fOrig, f' :| (f'' : fs)) allFs = if f /= f' then mapFs (fOrig, f'' :| fs) allFs else Right f'
      ...
      collectMoreFormulae formMap (Right (f, FSubst phi (Subst x t)) : rest) =
        case formMap !? phi of
          -- unify fs
          Just phiF ->
          ...
          Nothing -> case termMap !? t of
            Nothing -> collectMoreFormulae formMap rest
            -- backwards
            Just (_, t') ->
              let x' = case termMap !? x of
                    Just (_, Var x') -> x'
    ...
    refIsVisible
      (ruleLine, ruleAddr)
      (Right ((start, end), naContainedIn ruleAddr -> True)) =
        Just $
          "Error:\nline range "
            <> show start
            <> "-"
    ...
    lookupProofReference start end p =
      case (pIndexProof start end p, fromLineRange start end p, fromLineNo ruleLine p) of
        (_, _, Nothing) ->
          Left
        ...
        (Just prf, Just refAddr, Just ruleAddr) -> case refIsVisible
          (ruleLine, ruleAddr)
          (Right ((start, end), refAddr)) of
          Nothing -> Right prf
          Just err -> Left err

    -- tries to look up a 'LineReference'
    lookupLineReference :: Int -> Proof -> Either Text (Either Assumption Formula)
    lookupLineReference refLine p =
      case (pIndex refLine p, fromLineNo refLine p, fromLineNo ruleLine p) of
        (_, _, Nothing) ->
          Left
        ...
        (Just (Left a), Just refAddr, Just ruleAddr) ->
          maybeToLeft
            (Left a)
            (refIsVisible (ruleLine, ruleAddr) (Left (refLine, refAddr)))
        (Just (Right (Derivation f _)), Just refAddr, Just ruleAddr) ->
          maybeToLeft
            (Right f)
            (refIsVisible (ruleLine, ruleAddr) (Left (refLine, refAddr)))
...
checkFreshness p = pMapLinesWithAddr (goAssumption p) (const id) p
 where
  ...
  isFreshList v ((na, Left (fromWrapper -> Nothing, _)) : rest) =
    isFreshList v rest
  isFreshList v ((na, Left (fromWrapper -> Just a, _)) : rest) =
    if isFresh v a then isFreshList v rest else Just (na, Left a)
  isFreshList v ((na, Right (Derivation (fromWrapper -> Nothing) _)) : rest) =
    isFreshList v rest
  isFreshList v ((na, Right (Derivation (fromWrapper -> Just f) _)) : rest) =
    if isFresh v f then isFreshList v rest else Just (na, Right f)
...
regenerateSymbols p =
 ...
 where
  ...
  goAssumption (n, fsymbs, psymbs) a@(ParsedInvalid txt err (RawAssumption f), r) =
    let ((n', fsymbs', psymbs'), f') =
          goFormula (n, fsymbs, psymbs) (ParsedInvalid txt err f)
  ...
  goFormula (n, fsymbs, psymbs) a =
   ...
   where
    ...
    go n fsymbs psymbs txt form@(Opr name fs) =
      foldlM
        (\r f -> go n fsymbs psymbs txt f <&> combineWrappers r)
        (ParsedValid txt form)
        fs
     where
      combineWrappers :: Wrapper RawFormula -> Wrapper RawFormula -> Wrapper RawFormula
      combineWrappers (Unparsed _ err) _ =
        Unparsed txt err
      combineWrappers (ParsedInvalid{}) (Unparsed _ err) =
        Unparsed txt err
      combineWrappers (ParsedInvalid _ err _) (ParsedInvalid{}) =
        ParsedInvalid txt err form
      combineWrappers (ParsedInvalid _ err _) (ParsedValid{}) =
        ParsedInvalid txt err form
      combineWrappers (ParsedValid{}) (Unparsed _ err) =
        Unparsed txt err
      combineWrappers (ParsedValid{}) (ParsedInvalid _ err _) =
        ParsedInvalid txt err form
      combineWrappers (ParsedValid{}) (ParsedValid{}) =
        ParsedValid txt form
    go n fsymbs psymbs txt (Quantifier name variable formula) =
      go n fsymbs psymbs txt formula <&> (Quantifier name variable <$>)
