...
viewProof model =
  H.div_
    ...
    [ H.div_ [HP.class_ "line-nos-container"] lineNos
    , H.div_ [HP.class_ "formulae-container"] formulae
    , H.div_ [HP.class_ "rules-container"] rules
    ]
 where
  (lineNos, formulae, rules) = unzip3 $ pSerializeWithAddr goAssumption goDerivation p
  p = model ^. proof
  goLineNo :: NodeAddr -> View Model Action
  goLineNo na = H.p_ [HP.class_ "line-no"] [text $ ms (lineNoOr999 na p)]
  goAssumption :: NodeAddr -> Assumption -> (View Model Action, View Model Action, View Model Action)
  goAssumption na a =
    ( goLineNo na
    , ""
    , ""
    )
  goDerivation :: NodeAddr -> Derivation -> (View Model Action, View Model Action, View Model Action)
  goDerivation na d =
    ( goLineNo na
    , ""
    , ""
